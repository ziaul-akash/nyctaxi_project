...
df_trips.agg(
    max(df_trips.tpep_pickup_datetime),
    min(df_trips.tpep_pickup_datetime)
).display()

# COMMAND ----------

three_months_ago= get_month_start_n_months_ago(3)
df_trips= df_trips.filter(df_trips.tpep_pickup_datetime >= three_months_ago)
...
df_join_final.write.mode("append").saveAsTable("nyctaxi.02_silver.taxi_trips_enriched")
